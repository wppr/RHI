...
void main() {
	...
	if (1.0==depth) {
		image_out = textureLod(env_map_sampler, position_world.xyz, 0.0).rgb;
    ...
    vec3 q=result;
    float gamma=1.0/2.2;
  	image_out=vec3(pow(q.r,gamma),pow(q.g,gamma),pow(q.b,gamma));

  //	if(spe>0.24&&spe<0.26)
  //  	image_out=vec3(1.0);
    //image_out= albedo;
    //image_out=N;
    //image_out=vec3(ao_term);
    //image_out=mix(base_color,result,0.0);
    //image_out=base_color;

    //image_out=position_world.xyz;
    //image_out=normalize((ViewMatrix*vec4(1.0)).xyz);
   // image_out=(normalize(position_view)*0.5+0.5).xyz;
    //image_out=vec3(shadow_term);
   // image_out=shadowcoord.xyz/shadowcoord.w;
    //image_out=texture(shadow_map_sampler,texcoord).xyz;
}
